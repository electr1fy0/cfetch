...
export function CodeEditor({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const html = await codeToHtml(content || "// Start typing your template", {
          lang: language,
        ...
        if (!cancelled) {
          setHighlighted(
            `<pre><code>${escapeHtml(content || "// Start typing your template")}</code></pre>`,
          );
        }
      }
    };
  ...
  return (
    <div className={cn("grid gap-3", className)}>
      ...
      <label className="flex flex-col gap-1 text-xs text-stone-400">
        <span className="font-mono uppercase tracking-wide">Template</span>
